...
type PresencaResponse = {
  total: number;
  presente: number;
  data: (VotoCamaraEnriquecido | VotoSenadoEnriquecido)[];
  count: number;
};
...
export default async function PresencaPage({ params, searchParams }: Props) {
  ...
  const { total, presente, data: rows, count } = await apiFetch<PresencaResponse>(
    `/parlamentares/${casa}/${id}/presenca?limit=${PAGE_SIZE}&offset=${offset}`,
    3600,
  );
    ...
    <>
      ...
      <div className="border-b border-border-base">
        <div className="max-w-[1180px] mx-auto grid grid-cols-2 sm:grid-cols-4">
          {kpiRows.map(({ label, valor, delta, pos }, i) => (
      ...
      <div className="max-w-[1180px] mx-auto px-4 sm:px-8 py-8">
        <div className="flex items-center justify-between mb-4">
          ...
          <span className="text-[13px] text-text-muted">{count.toLocaleString("pt-BR")} registros</span>
        </div>
          ...
          {casa === "camara" ? (
            <>
              <div className="hidden sm:grid grid-cols-[120px_1fr_110px_130px] gap-4 px-4 py-3 bg-surface-alt text-xs font-bold text-text-body uppercase tracking-[0.03em]">
                <span>Data</span><span>Votação</span><span>Voto</span><span>Status</span>
              </div>
              <div className="divide-y divide-border-base">
                {(rows as VotoCamaraEnriquecido[]).map((v, i) => (
                  <div key={`${v.votacao_id}-${i}`}>
                    <div className="sm:hidden px-4 py-3">
                      ...
                      {v.tipo_voto && <p className="text-xs text-text-body mt-1">Voto: <strong>{v.tipo_voto}</strong></p>}
            ...
            <>
              <div className="hidden sm:grid grid-cols-[110px_120px_1fr_110px_160px] gap-4 px-4 py-3 bg-surface-alt text-xs font-bold text-text-body uppercase tracking-[0.03em]">
                <span>Data</span><span>Matéria</span><span>Descrição</span><span>Voto</span><span>Status</span>
              </div>
              <div className="divide-y divide-border-base">
                {(rows as VotoSenadoEnriquecido[]).map((v, i) => (
                  <div key={`${v.codigo_sessao_votacao}-${i}`}>
                    <div className="sm:hidden px-4 py-3">
                      ...
                      {v.identificacao && <p className="text-xs text-brand-blue font-bold mb-0.5">{v.identificacao}</p>}
                      <p className="text-sm text-text-strong font-semibold leading-snug line-clamp-2">{v.descricao_votacao ?? "—"}</p>
                      {v.sigla_voto && <p className="text-xs text-text-body mt-1">Voto: <strong>{v.sigla_voto}</strong></p>}
                    </div>
                    <div className="hidden sm:grid grid-cols-[110px_120px_1fr_110px_160px] gap-4 px-4 py-3 items-start">
                      ...
                      <p className="text-[13px] text-text-strong font-semibold leading-snug line-clamp-2">{v.descricao_votacao ?? "—"}</p>
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-between pt-5">
            <span className="text-[13px] text-text-muted">
              {offset + 1}–{Math.min(offset + PAGE_SIZE, count)} de {count.toLocaleString("pt-BR")}
            </span>
